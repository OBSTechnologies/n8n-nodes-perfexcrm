...
Connect [PerfexCRM](https://www.perfexcrm.com/) to [n8n](https://n8n.io/) with this **verified n8n community node** — installable in one click directly from the n8n canvas, on n8n Cloud or self-hosted — and build no-code / low-code workflow automations across your entire CRM. It ships with **19 PerfexCRM resources**, **171 REST API operations**, and a trigger node with **99 real-time webhook events** — enough coverage to automate leads, tickets, invoices, projects, contracts, subscriptions, and more without writing glue code. Whether you want to sync PerfexCRM data to Slack, Google Sheets, HubSpot, or Mailchimp, push webhook-driven events into any of n8n's 400+ integrations, or orchestrate multi-step CRM workflows, this node is a complete PerfexCRM automation toolkit.
...
- [PerfexCRM API Operations (19 Resources, 171 Operations)](#perfexcrm-api-operations-19-resources-171-operations)
- [PerfexCRM Trigger Node — 99 Real-Time Webhook Events](#perfexcrm-trigger-node--99-real-time-webhook-events)
...
- **Trigger workflows from 99 PerfexCRM webhook events** — from `lead.created` and `invoice.paid` to `contract.expiring` and `task.overdue`.
...
## PerfexCRM API Operations (19 Resources, 171 Operations)

### PerfexCRM Action Node — REST API Coverage

**19 resources** with **171 operations** covering the entire PerfexCRM API.
...
99 webhook events across 18 categories — leads, customers, invoices, tickets, tasks, contracts, projects, proposals, estimates, payments, subscriptions, and more. See the [full event table](#supported-events-99-webhook-events).
